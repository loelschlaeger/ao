...
#' \code{\link[optimizeR]{set_optimizer}}) for a custom optimizer.
#' The arguments \code{lower} and \code{upper} are ignored in this case.
#'
#' @return
...
#'
#' ### custom optimizer
#' custom_opt <- optimizeR::set_optimizer(
...
set_f <- function(f, ..., npar, lower = -Inf, upper = Inf, check = FALSE,
  ...
  if (is.null(optimizer)) {
    ...
    optimizer <- optimizeR::set_optimizer_optim("lower" = lower, "upper" = upper)
  }
  if (!inherits(optimizer, "optimizer")) {
    stop("'optimizer' must be generated by 'optimizeR::set_optimizer()'.",
         call. = FALSE)
  }
  if (!is.logical(check)) {
